/**
 * public_html/js/team.controller.js
 * 
 */
...
const TeamController = {
    ...
    loadData: async () => {
        TeamController.state.filterDays = document.getElementById('team-date-filter').value;
        const res = await Utils.callApi('fetch_team_stats', { days: TeamController.state.filterDays });

        if (res.status === 'success') {
            TeamController.state.rawData = res.data;
            if (res.totalStats && TeamController.state.currentView === 'departments') {
            ...
            if (TeamController.state.currentView === 'dept-detail') {
                if (TeamController.state.role === 'MANAGER') {
                    ...
                    const dept = res.data.find(d => d.id == TeamController.state.selectedDeptId);
    ...
    openDepartment: (deptId) => {
        ...
        document.getElementById('team-date-filter')?.parentElement.classList.remove('hidden');
        ...
        if (deptData) {
            ...
            setTimeout(() => {
                const y = document.getElementById('view-dept-detail').getBoundingClientRect().top + window.scrollY - 100;
    ...
    openMember: (userId) => {
        ...
        document.getElementById('team-date-filter')?.parentElement.classList.add('hidden');
    ...
    goBack: () => {
        ...
        if (TeamController.state.currentView === 'member-detail') {
            ...
            if (dateFilter && dateFilter.parentElement) dateFilter.parentElement.classList.remove('hidden');

            if (TeamController.state.role === 'MANAGER') {
                document.getElementById('btn-team-back').classList.add('hidden');
                document.getElementById('team-page-title').innerText = "PHÒNG BAN CỦA TÔI";
            } else {
                const dept = TeamController.state.rawData.find(d => d.id == TeamController.state.selectedDeptId);
                if (dept) document.getElementById('team-page-title').innerText = dept.name.toUpperCase();
            }
            ...
            if (TeamController.state.rawData) TeamDeptService.renderDepartmentChart(TeamController.state.rawData);

            document.getElementById('team-page-title').innerText = "TỔNG QUAN ĐỘI NHÓM";
        }
    ...
    openRatingModal: (staffId, staffName, preSelectType = 'LIKE') => {
        ...
        document.getElementById('rating-avatar-preview').innerText = staffName.charAt(0);
    ...
    closeRatingModal: () => { document.getElementById('rating-modal')?.classList.add('hidden'); },
    ...
    submitRating: async (event) => {
        ...
        const res = await Utils.callApi('evaluate_staff', { staff_id: staffId, type: type, note: note });
        if (res.status === 'success') {
    ...
    loadMemberTasks: async (userId) => {
        document.getElementById('view-member-detail').innerHTML = '<div class="py-32 flex flex-col items-center justify-center text-slate-300"><i data-lucide="loader-2" width="40" class="animate-spin mb-4"></i><span class="text-xs font-bold uppercase tracking-widest">Đang tải...</span></div>';
        const res = await Utils.callApi('fetch_member_tasks', { user_id: userId, days: TeamController.state.filterDays });
        if (res.status === 'success') {
    ...
    changeFilter: () => {
        if (TeamController.state.currentView === 'member-detail') TeamController.loadMemberTasks(TeamController.state.selectedUserId);
