...
class TasksController {
   ...
   setupAssigneeToggle(form) {
        ...
        if (searchInput) {
            searchInput.oninput = () => {
                ...
                const filtered = this.staffList.filter(s => s.fullName.toLowerCase().includes(term));
                this.renderAssigneeList(filtered, listContainer, idInput);
            };
        }
        
        document.addEventListener('click', (e) => {
            if (assigneeSection && !assigneeSection.classList.contains('hidden') && !e.target.closest('#assign-toggle-wrapper')) {
                assigneeSection.classList.add('hidden');
                const icon = document.getElementById('assign-toggle-icon');
                if(icon) icon.style.transform = 'rotate(0deg)';
            }
   ...
   renderAssigneeList(list, container, idInput) {
        ...
        if (list.length === 0) {
            container.innerHTML = '<div class="text-center py-6 text-slate-400 text-[10px] font-black uppercase tracking-widest italic">Không tìm thấy nhân viên</div>';
            return;
        }
   ...
   selectAssignee(id) {
    ...
    updateAssigneeDisplay(id) {
        ...
        if (id == user.id) {
            btnContainer.className = "flex items-center justify-between p-3.5 rounded-2xl border-2 border-slate-100 bg-slate-50 hover:bg-white hover:border-indigo-200 cursor-pointer transition-all group";
            ...
            if (selectedUser) {
                ...
                const avatarSrc = Utils.getAvatar(selectedUser.avatar, selectedUser.fullName, 128);
                const deptName = selectedUser.dept_name || 'Nhân sự';

                btnContainer.className = "flex items-center justify-between p-2.5 rounded-2xl border-2 border-indigo-500 bg-indigo-50/50 cursor-pointer transition-all shadow-sm";
                btnContainer.innerHTML = `
                    <div class="flex items-center gap-3">
    ...
    async handleDrop(e, targetQuadrant) {
        ...
        if (taskEl) {
            ...
            if (res.status === 'success') {
                if (window.Utils) window.Utils.showToast('Đã di chuyển công việc!', 'success');
                await this.loadTasks();
            } else {
                if (window.Utils) window.Utils.showToast("Lỗi di chuyển: " + res.message, "error");
                await this.loadTasks();
            }
        }
    }
    ...
    async handleCreateTask(e) {
        ...
        const user = this.getCurrentUser();
        if (window.Utils) window.Utils.toggleLoading('btn-create-task', true, 'Đang lưu...');
        ...
        const res = await taskService.createTask({
            title: title,
            quadrant: form.querySelector('input[name="quadrant"]:checked')?.value || 'do_first',
            assigneeId: finalAssigneeId,
            startTime: form.start_time.value,
            endTime: form.end_time.value
        });

        if (window.Utils) window.Utils.toggleLoading('btn-create-task', false, 'THÊM CÔNG VIỆC');
    ...
    async updateStatus(taskId, status) {
        const res = await this._callApi('update_task_status', { id: taskId, status: status });
        if (res.status === 'success') {
            await this.loadTasks();
            if (status == 3) {
    ...
    processTasks(tasks) {
        ...
        return tasks.filter(t => {
            ...
            if (t.status == 3 || t.status == 4) {
                ...
                const createDateStr = t.createdAt ? new Date(new Date(t.createdAt).getTime() - offset).toISOString().split('T')[0] : '';

                // Lấy chuỗi ngày YYYY-MM-DD của ngày cập nhật cuối cùng
                const updateDateStr = t.updatedAt ? new Date(new Date(t.updatedAt).getTime() - offset).toISOString().split('T')[0] : '';
    ...
    async saveTitle(taskId) {
        ...
        if (!input) return;
        const res = await this._callApi('update_task_title', { id: taskId, title: input.value });
        if (res.status === 'success') await this.loadTasks();
    }

    _normalizeTask(t) {
        return {
            id: t.id, title: t.title, status: Number(t.status),
    ...
    getCurrentUser() { try { return JSON.parse(localStorage.getItem('current_session_user')); } catch (e) { return null; } }
    enableEditMode(id) { document.getElementById(`view-mode-${id}`)?.classList.add('hidden'); document.getElementById(`edit-mode-${id}`)?.classList.remove('hidden'); document.getElementById(`input-title-${id}`)?.focus(); }
    cancelEdit(id) { document.getElementById(`view-mode-${id}`)?.classList.remove('hidden'); document.getElementById(`edit-mode-${id}`)?.classList.add('hidden'); }
...
document.addEventListener('DOMContentLoaded', () => { new TasksController().init(); });