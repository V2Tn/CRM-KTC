/**
 * js/services/schedule/leave.service.js
 */
const LeaveService = {
    openForm: async (defaultDate) => {
        ...
        document.getElementById('leave-follower-select-box').classList.add('hidden');

        if (typeof selectCustomDropdown === 'function') selectCustomDropdown('leave-type', 'PHÉP NĂM', 'Phép năm');
        ...
        try {
            ...
            if (resStaff.data) {
                resStaff.data.forEach(u => {
                    ...
                    const isBGD = (u.dept_name && u.dept_name.includes('Giám Đốc')) ? 'checked' : '';
            ...
            if (resMgr.data) {
                resMgr.data.forEach(m => {
                    ...
                    if (m.current_dept_id == myDeptId) {
                        ...
                        if (typeof selectCustomDropdown === 'function') selectCustomDropdown('leave-manager', m.id, displayLabel);
                    }
                });
            }
    ...
    searchFollowers: () => {
        const keyword = document.getElementById('search-follower-input').value.toLowerCase();
        document.querySelectorAll('.follower-item').forEach(item => {
    ...
    toggleFollowerList: () => { document.getElementById('leave-follower-select-box').classList.toggle('hidden'); },

    updateFollowerTags: () => {
        const container = document.getElementById('leave-selected-followers-tags');
        const checkboxes = document.querySelectorAll('input[name="leave_followers[]"]:checked');
        let html = '';
    ...
    removeFollower: (id) => {
        const cb = document.querySelector(`input[name="leave_followers[]"][value="${id}"]`);
        if (cb) {
            cb.checked = false;
            LeaveService.updateFollowerTags();
    ...
    closeForm: () => document.getElementById('leave-form-modal').classList.add('hidden'),
    ...
    submitForm: async (e) => {
        ...
        if (!managerId) return Utils.showToast('Vui lòng chọn người duyệt!', 'error');
        if (new Date(start) > new Date(end)) return Utils.showToast('Ngày bắt đầu không được lớn kết thúc!', 'error');

        const followers = Array.from(document.querySelectorAll('input[name="leave_followers[]"]:checked')).map(cb => cb.value);

        Utils.showToast('Đang gửi đơn...', 'info');
        const res = await Utils.callApi('create_leave_request', { start_date: start, end_date: end, reason: reason, leave_type: leaveType, manager_id: managerId, followers: followers });
