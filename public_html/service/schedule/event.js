...
const EventService = {
  ...
  toggleUserSelect: () => {
    ...
    if (type === "SPECIFIC") {
      ...
      document
        .querySelectorAll('input[name="event_specific_users[]"]')
  ...
  updateUserTags: () => {
    ...
    const checkboxes = document.querySelectorAll(
      'input[name="event_specific_users[]"]:checked',
    );
    let html = "";
  ...
  searchUsers: () => {
    const keyword = document
      .getElementById("search-event-user-input")
      .value.toLowerCase();
  ...
  loadUsers: async () => {
    ...
    try {
      ...
      if (users.length > 0) {
        ...
        container.innerHTML =
          '<div class="text-xs text-center text-slate-400 py-4 font-bold">Không tìm thấy nhân viên nào!</div>';
      }
    } catch (error) {}
  },
  ...
  submitForm: async (e) => {
    ...
    if (notifyType === "SPECIFIC") {
      ...
      if (notifyUsers.length === 0)
        return Utils.showToast("Vui lòng tick chọn ít nhất 1 người!", "error");
    }
    ...
    try {
      const res = await Utils.callApi("create_event", {
        title,
