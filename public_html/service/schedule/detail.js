...
const DetailService = {
  ...
  open: async (type, id) => {
    ...
    if (
      ...
      typeof ScheduleController.closeMoreEventsModal === "function"
    ) {
    ...
    document
      .getElementById("detail-default-actions")
      ?.classList.remove("hidden");
    document.getElementById("detail-modal").classList.remove("hidden");

    try {
      const res = await Utils.callApi("get_schedule_detail", {
        type: type,
        id: id,
      });
      ...
      if (res && res.status === "success" && res.data) {
        ...
        if (type === "EVENT") {
          document.getElementById("detail-content").innerText =
            data.content || "Không có ghi chú thêm.";
          document
            .getElementById("detail-event-info")
            .classList.remove("hidden");
          document.getElementById("detail-creator").innerText =
            data.creator_name;
          document.getElementById("detail-event-start").innerText =
            data.start_time;
          document.getElementById("detail-event-end").innerText = data.end_time;

          badge.className =
            "text-[10px] font-bold uppercase mt-2 px-3 py-1 rounded-md inline-block bg-indigo-100 text-indigo-600";
          badge.innerText = "SỰ KIỆN CÔNG TY";
        } else if (type === "LEAVE") {
          DetailService.currentLeaveId = data.id;
          document
            .getElementById("detail-leave-info")
            .classList.remove("hidden");

          // [ĐÃ SỬA] In nhãn Loại Phép ra ngay lập tức
          badge.className =
            "text-[10px] font-bold uppercase mt-2 px-3 py-1 rounded-md inline-block bg-rose-100 text-rose-600";
          badge.innerText = `LOẠI: ${data.leave_type || "PHÉP NĂM"}`;

          // Đổ dữ liệu text
          document.getElementById("detail-leave-requester").innerText =
            data.requester_name || "--";
          document.getElementById("detail-leave-start").innerText =
            data.start_time || "--";
          document.getElementById("detail-leave-end").innerText =
            data.end_time || "--";
          document.getElementById("detail-leave-manager").innerText =
            data.creator_name || "Chưa rõ";
          document.getElementById("detail-leave-followers").innerText =
            data.followers_name || "Không có";
          ...
          if (data.status === "APPROVED") {
            ...
            statusEl.className =
              "text-[10px] font-black px-2 py-1 rounded bg-emerald-100 text-emerald-600 uppercase";
          } else if (data.status === "REJECTED") {
            statusEl.innerText = "TỪ CHỐI";
            statusEl.className =
              "text-[10px] font-black px-2 py-1 rounded bg-rose-100 text-rose-600 uppercase";
          } else {
            statusEl.innerText = "CHỜ DUYỆT";
            statusEl.className =
              "text-[10px] font-black px-2 py-1 rounded bg-amber-100 text-amber-600 uppercase";
          }
          ...
          const reasonWrapper = document.getElementById(
            "detail-leave-reason-wrapper",
          );
          const noteWrapper = document.getElementById(
            "detail-leave-manager-note-wrapper",
          );
          ...
          if (data.has_permission) {
            ...
            if (data.manager_note && data.manager_note.trim() !== "") {
              ...
              document.getElementById(
                "detail-leave-manager-note-text",
              ).innerText = data.manager_note;
            } else {
          ...
          if (
            ...
            document
              .getElementById("detail-leave-actions")
              .classList.remove("hidden");
            document
              .getElementById("detail-leave-actions")
              .classList.add("flex");
  ...
  processLeave: async (statusAction) => {
    ...
    const managerNote = document.getElementById(
      "detail-manager-note-input",
    ).value;
  ...
  checkUrlParams: () => {
    ...
    if (urlParams.get("view_type") && urlParams.get("view_id")) {
      ...
      window.history.replaceState(
        {},
        document.title,
        window.location.pathname + "?tab=schedule",
      );
    }
  },
};
