...
const NotificationController = {
  ...
  init: () => {
      ...
      if (data.receivers && data.receivers.includes(myUserId)) {
        ...
        if (!currentTab || currentTab === "tasks") {
          if (
            ...
            typeof window.TaskController.loadTasks === "function"
          ) {
        ...
        if (currentTab === "schedule") {
          if (
            ...
            typeof ScheduleController.renderCalendar === "function"
          ) {
    ...
    document.addEventListener("click", (e) => {
      ...
      const btn = document.querySelector(
        '[onclick="NotificationController.toggle()"]',
      );
      if (
        dropdown &&
      ...
      if (res.notifications && res.notifications.length > 0) {
        ...
        if (NotificationController.lastNotiId === 0) {
          ...
          if (topNoti.type === "TASK_CANCEL") {
            ...
            topNoti.type === "TASK_REDO" ||
            topNoti.type === "NEW_TASK" ||
            topNoti.type === "TASK_REMIND"
          ) {
  ...
  handleAutoReload: (noti) => {
    ...
    if (noti.type === "EVALUATION") {
      if (
        window.TeamController &&
        typeof window.TeamController.loadData === "function"
      ) {
  ...
  renderList: (list) => {
    ...
    if (!list || list.length === 0) {
      container.innerHTML =
        '<div class="p-6 text-center text-slate-400 text-xs font-bold">Không có thông báo nào</div>';
      return;
    }
    ...
    list.forEach((item) => {
      ...
      const dotHtml = isUnread
        ? '<div class="w-2 h-2 rounded-full bg-rose-500 shrink-0 shadow-sm shadow-rose-200"></div>'
        : "";
  ...
  markRead: async (notiId = null, relatedId = null, type = "") => {
    ...
    if (taskTypes.includes(type) && relatedId) {
      ...
      if (urlParams.get("tab") === "schedule") {
        if (typeof DetailService !== "undefined")
          DetailService.open("EVENT", relatedId);
        else
      ...
      if (urlParams.get("tab") === "schedule") {
        if (typeof DetailService !== "undefined")
          DetailService.open("LEAVE", relatedId);
        else
