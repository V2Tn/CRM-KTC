...
window.renderTasksGlobal = function (tasks, staffList = []) {
  ...
  quadrants.forEach((q) => {
    ...
    if (el) {
      ...
      el.className =
        "p-4 flex-1 overflow-y-auto scrollbar-hide space-y-3 max-h-[550px]";
  ...
  if (listViewContainer) {
    ...
    if (tasks && tasks.length > 0) {
      tasks.forEach((task) => {
        listViewContainer.insertAdjacentHTML(
          "beforeend",
  ...
  if (
    ...
    typeof window.TaskController.initDragAndDrop === "function"
  ) {
...
function getUserLabels(task) {
  ...
  return {
    creator:
      Number(task.createdById) === currentUserId
        ? "Tôi"
        : task.createdByLabel || "System",
    assignee:
      Number(task.assigneeId) === currentUserId
        ? "Tôi"
        : task.assigneeLabel || "NV",
  };
}
...
function createTaskListHTML(task) {
  ...
  const btnBase =
    "w-9 h-9 flex items-center justify-center rounded-full transition-all active:scale-90 border shadow-sm";
  let btns = "";
  if (!isFinished) {
...
function createTaskHTML(task, staffList) {
  ...
  const stateConfig = {
    ...
    2: {
      label: "ĐANG LÀM",
      class: "bg-indigo-500 text-white border-transparent animate-pulse",
    },
    3: {
      label: "HOÀN THÀNH",
  ...
  const isFinished = statusId == 3 || statusId == 4;
  const textStyle = isFinished
    ? "line-through text-slate-400"
    : "text-slate-800";
  const cardOpacity = isFinished
    ? "opacity-70 bg-slate-50"
    : "bg-white hover:shadow-lg";

  const startTimeStr = task.startTime
    ? `${fmtTime(task.startTime)} ${fmt(task.startTime)}`
    : fmt(task.createdAt);
  ...
  let creatorLabel =
    cID === currentUserId ? "TÔI" : task.createdByLabel || "System";
  let assigneeLabel =
    aID === currentUserId ? "TÔI" : task.assigneeLabel || "NV";
  ...
  let buttons = "";
  const btnClass =
    "w-8 h-8 flex items-center justify-center rounded-lg border transition-all active:scale-95 shadow-sm";
...
window.selectCustomDropdown = function (
  prefix,
  value,
  label,
  callbackStr = null,
...
document.addEventListener("click", (e) => {
  if (!e.target.closest(".custom-dropdown-container")) {
    document.querySelectorAll(".custom-dropdown-menu").forEach((d) => {
      ...
      const icon = d.previousElementSibling?.querySelector(
        '[data-lucide="chevron-down"]',
      );
      if (icon) icon.style.transform = "rotate(0deg)";
    });
