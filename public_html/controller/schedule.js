...
const ScheduleController = {
  ...
  toggleMonthPicker: (e) => {
    ...
    if (!popup.classList.contains("hidden")) {
      ScheduleController.pickerYear =
        ScheduleController.state.currentDate.getFullYear();
      ScheduleController.renderMonthPicker();
    }
  },
  changePickerYear: (offset) => {
    ScheduleController.pickerYear += offset;
    ScheduleController.renderMonthPicker();
  },
  renderMonthPicker: () => {
    document.getElementById("picker-year-display").innerText =
      ScheduleController.pickerYear;
    ...
    for (let i = 0; i < 12; i++) {
      const isSelected =
        i === currentM && ScheduleController.pickerYear === currentY;
  ...
  renderCalendar: async () => {
    ...
    grid.innerHTML =
      '<div class="col-span-7 text-center py-10 text-slate-400 font-bold animate-pulse">Đang tải dữ liệu lịch...</div>';
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      let bgClass =
        "bg-white border-slate-100/80 hover:border-indigo-200 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:-translate-y-0.5";
      let textClass =
        "text-slate-500 bg-slate-50 group-hover:bg-indigo-50 group-hover:text-indigo-600";

      if (holidayName && !isToday) {
        bgClass =
          "bg-rose-50/30 border-rose-100 hover:border-rose-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:-translate-y-0.5";
        textClass =
          "text-rose-500 bg-rose-50 group-hover:bg-rose-100 group-hover:text-rose-600";
      }

      if (isToday) {
        bgClass =
          "bg-gradient-to-br from-indigo-50/80 to-white border-indigo-200 ring-4 ring-indigo-50/50 shadow-sm hover:-translate-y-0.5 hover:shadow-md";
        textClass =
          "bg-indigo-600 text-white shadow-md shadow-indigo-200 group-hover:bg-indigo-700 group-hover:text-white";
      } else if (isWeekend && !holidayName) {
        textClass =
          "text-rose-400 bg-rose-50/50 group-hover:bg-rose-100 group-hover:text-rose-600";
      }
  ...
  openActionModal: (dateStr, directAction = null, displayDateStr = null) => {
    ...
    if (directAction) {
      ScheduleController.proceedToForm(directAction);
      return;
    }
    document.getElementById("schedule-action-modal").classList.remove("hidden");
  },
  closeActionModal: () => {
    document.getElementById("schedule-action-modal").classList.add("hidden");
  },
