...
class DepartmentController {
  ...
  handleManagerSelection(newId, newName) {
    const currentSelectedId = document.getElementById(
      "dept-manager-hidden-val",
    ).value;
    const currentSelectedName = document.getElementById(
      "label-manager-selected",
    ).innerText;
    ...
    if (currentSelectedId && currentSelectedId !== newId && newId !== "") {
      ...
      document.getElementById("current-manager-name").innerText =
        currentSelectedName;
  ...
  async openDeptModal(id = null) {
    ...
    if (id) {
      ...
      if (dept) {
        ...
        this.currentDeptMembers =
          resMem.status === "success" ? resMem.data : [];
      }
    } else {
  ...
  checkManagerConflict(selectedId) {
    ...
    if (
      manager &&
      manager.current_dept_id &&
      manager.current_dept_id != currentDeptId
    ) {
      warningText.innerText = `${manager.username} đang quản lý ${manager.current_dept_name}`;
      warning.classList.remove("hidden");

      window.Utils.showToast(
        `Cảnh báo: ${manager.fullName} đang quản lý phòng khác!`,
        "error",
      );
  ...
  async saveDept(event) {
    ...
    try {
      ...
      if (res.status === "success") {
        if (window.Utils)
          window.Utils.showToast(
            data.id ? "Đã cập nhật!" : "Thêm thành công!",
            "success",
          );
        this.closeDeptModal();
        this.loadDepartmentList();
  ...
  renderDepartmentList(departments) {
    ...
    departments.forEach((d) => {
      const deleteBtn = isManager
        ? ""
        : `<button onclick="DepartmentController.deleteDept(${d.id}, '${d.name}')" class="w-8 h-8 flex items-center justify-center rounded-xl text-slate-300 hover:bg-rose-50 hover:text-rose-500 transition-all"><i data-lucide="trash-2" width="16"></i></button>`;
  ...
  renderCurrentMembers() {
    ...
    if (this.currentDeptMembers.length === 0) {
      container.innerHTML =
    ...
    this.currentDeptMembers.forEach((u) => {
      const avatarSrc = u.avatar
        ? u.avatar
        : `https://ui-avatars.com/api/?name=${encodeURIComponent(u.fullName)}&background=random&size=64`;
  ...
  renderAvailableMembers() {
    ...
    const filtered = this.availableUsers.filter(
      (u) =>
    ...
    if (filtered.length === 0) {
      ...
      filtered.forEach((u) => {
        const deptBadge = u.current_dept_name
          ? `<span class="text-[9px] text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded ml-1">${u.current_dept_name}</span>`
  ...
  toggleAddMemberView(show) {
    ...
    if (show) overlay.classList.remove("translate-x-full");
  ...
  async executeDelete() {
    ...
    let res;
    if (type === "dept") {
      res = await deptService.delete(id);
    }
