  ...
  login: async (event) => {
    ...
    try {
      ...
      if (res.status === "success") {
        ...
        AuthController.showErrorModal(
          "Tài khoản hoặc mật khẩu không chính xác. Vui lòng kiểm tra lại hoặc liên hệ quản trị viên!",
        );
      }
    } catch (e) {
      if (overlay) overlay.classList.add("hidden");
      AuthController.showErrorModal(
        "Hiện hệ thống đang gặp sự cố. Vui lòng thử lại sau!",
      );
    } finally {
      if (window.Utils)
        Utils.toggleLoading("btn-submit", false, "ĐANG NHẬP NGAY");
    }
  },
