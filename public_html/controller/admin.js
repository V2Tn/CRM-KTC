...
class AdminController {
  ...
  renderDepartmentList(departments) {
    ...
    departments.forEach((d) => {
      ...
      const deleteBtn = isManager
        ? ""
        : `<button onclick="AdminController.deleteDept(${d.id}, '${d.name}')" class="w-8 h-8 flex items-center justify-center rounded-xl text-slate-300 hover:bg-rose-50 hover:text-rose-500 transition-all"><i data-lucide="trash-2" width="16"></i></button>`;
  ...
  openStaffModal(id = null) {
    ...
    if (id) {
      ...
      if (avatarPreview)
        avatarPreview.src = "https://via.placeholder.com/128?text=USER";
  ...
  openDeleteModal(type, id, name = "mục này") {
    ...
    if (modal && nameEl) {
      nameEl.innerText =
        type === "staff" ? `nhân viên "${name}"` : `phòng ban "${name}"`;
  ...
  async executeDelete() {
    ...
    if (res && res.status === "success") {
      ...
      if (type === "staff") this.loadStaffList();
      else {
        this.loadDepartmentList();
        this.loadDepartmentsToSelect();
      }
