...
const ReportController = {
  ...
  switchDiary(type) {
    ...
    if (btnMy)
      btnMy.className = `px-4 py-2 text-[10px] font-black rounded-lg ${type === "my" ? "bg-white shadow-sm text-indigo-600" : "text-slate-400 hover:text-slate-600"}`;
    if (btnAssign)
      btnAssign.className = `px-4 py-2 text-[10px] font-black rounded-lg ${type === "assign" ? "bg-white shadow-sm text-indigo-600" : "text-slate-400 hover:text-slate-600"}`;

    // 1. Render lại danh sách Task dưới cùng
    this.renderDiary(
      type === "my" ? this.allData.myTasks : this.allData.assignedTasks,
    );

    // 2. [THÊM MỚI] Render lại Biểu đồ dựa trên tab đang chọn!
    this.renderChart(
      type === "my" ? this.allData.myChartData : this.allData.assignChartData,
    );
    ...
    if (subLabel) {
      subLabel.innerText =
        type === "my"
          ? "DỮ LIỆU CÔNG VIỆC TÔI LÀM"
          : "DỮ LIỆU CÔNG VIỆC TÔI GIAO";
    }
  },
  ...
  createReportTaskCard(task) {
    ...
    if (isDone) {
      ...
      statusBadge =
        '<span class="px-2 py-0.5 rounded text-[9px] font-black uppercase bg-emerald-100 text-emerald-600">Hoàn thành</span>';
      iconHtml =
        '<div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0"><i data-lucide="check-check" width="16"></i></div>';
    } else if (isCancel) {
      bgClass = "bg-slate-50 hover:bg-slate-100 opacity-70";
      borderLeft = "border-l-4 border-slate-400";
      statusBadge =
        '<span class="px-2 py-0.5 rounded text-[9px] font-black uppercase bg-slate-200 text-slate-500">Đã hủy</span>';
      iconHtml =
        '<div class="w-8 h-8 rounded-full bg-slate-200 text-slate-500 flex items-center justify-center shrink-0"><i data-lucide="x" width="16"></i></div>';
    } else {
      bgClass = "bg-white hover:bg-indigo-50/50";
      borderLeft = "border-l-4 border-indigo-400";
      statusBadge =
        '<span class="px-2 py-0.5 rounded text-[9px] font-black uppercase bg-indigo-100 text-indigo-600">Chưa xong</span>';
      iconHtml =
        '<div class="w-8 h-8 rounded-full bg-indigo-50 text-indigo-500 flex items-center justify-center shrink-0"><i data-lucide="clock" width="16"></i></div>';
    }
    ...
    let completedAtHtml = "";
    if (isDone) {
      const durationStr = calcDuration(
  ...
  renderDiary(tasks) {
    ...
    const groups = {};
    tasks.forEach((t) => {
      const dateStr = new Date(t.startTime).toLocaleDateString("vi-VN", {
        day: "2-digit",
        month: "2-digit",
      });
      ...
      if (taskCard) {
        ...
        setTimeout(() => {
          ...
          taskCard.style.boxShadow =
            "0 0 0 4px #818cf8, 0 10px 15px -3px rgba(0, 0, 0, 0.1)";
