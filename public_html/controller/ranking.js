...
const RankingController = {
  ...
  loadData: async (period) => {
    ...
    document.querySelectorAll(".ranking-period-btn").forEach((btn) => {
      btn.className =
        "ranking-period-btn px-5 py-2.5 text-xs font-black rounded-xl transition-all bg-white text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 border border-slate-100 shadow-sm flex items-center gap-2";
      if (btn.id === `btn-period-${period}`) {
        btn.className =
          "ranking-period-btn px-5 py-2.5 text-xs font-black rounded-xl transition-all bg-indigo-600 text-white shadow-md shadow-indigo-200 flex items-center gap-2";
      }
    });
    ...
    if (top3Container)
      top3Container.innerHTML =
        '<div class="w-full text-center animate-pulse text-indigo-300 font-bold text-xs uppercase tracking-widest">Đang tải dữ liệu...</div>';
    ...
    if (res.status === "success") {
      ...
      if (top3Container)
        top3Container.innerHTML = `<div class="w-full text-center text-rose-500 font-black uppercase text-sm bg-rose-50 p-4 rounded-xl border border-rose-100">Lỗi: ${res.message}</div>`;
    }
  },

  loadCustomMonth: async (monthValue, monthNumber) => {
    document.getElementById("ranking-custom-label").innerText =
      `Tháng ${monthNumber}/${RankingController.pickerYear}`;
    toggleCustomDropdown("ranking-month-dropdown");
    ...
    document.getElementById("btn-period-custom").className =
      "ranking-period-btn px-5 py-2.5 text-xs font-black rounded-xl transition-all bg-indigo-600 text-white shadow-md shadow-indigo-200 flex items-center gap-2";
    ...
    const res = await Utils.callApi("fetch_leaderboard", {
      period: monthValue,
    });
    if (res.status === "success") {
      RankingController.renderUI(res.data);
  ...
  showUserDetails: async (userId, encodedName, encodedAvatar) => {
    ...
    content.innerHTML =
      '<div class="text-center py-10 animate-pulse font-bold text-indigo-400 text-sm uppercase tracking-widest"><i data-lucide="loader-2" class="animate-spin inline-block w-6 h-6 mb-2"></i><br>Đang lấy dữ liệu...</div>';
    if (window.lucide) lucide.createIcons();
    ...
    if (res.status === "success") {
      ...
      const onTimePctWidth =
        totalResolved > 0 ? (d.on_time / totalResolved) * 100 : 0;
      const latePctWidth =
        totalResolved > 0 ? (d.late / totalResolved) * 100 : 0;
      const cancelPctWidth =
        totalResolved > 0 ? (d.canceled / totalResolved) * 100 : 0;
      ...
      if (d.total_evals === -1) {
        ...
        let itemsHtml = "";
        const colorMap = {
          Tốt: "text-emerald-500",
          "Xuất sắc": "text-yellow-500",
          Kém: "text-rose-500",
        };
        ...
        keys.forEach((key, idx) => {
          ...
          const borderClass =
            idx < keys.length - 1 ? "border-r border-slate-100" : "";
  ...
  renderUI: (data) => {
    ...
    if (!data || data.length === 0) {
      top3Container.innerHTML =
        '<div class="col-span-3 text-center py-10 text-slate-400 font-bold uppercase tracking-widest text-xs">Chưa có dữ liệu đánh giá</div>';
    ...
    order.forEach((index) => {
      ...
      const colorClass = isTop1
        ...
        : rank === 2
          ...
          : "from-amber-600 to-orange-700 shadow-orange-200/50";
    ...
    if (rest.length === 0) {
      listContainer.innerHTML =
        '<div class="text-center py-10 text-slate-400 font-bold uppercase tracking-widest text-xs">Không có nhân sự nào khác</div>';
      return;
    }
    let listHtml = "";
